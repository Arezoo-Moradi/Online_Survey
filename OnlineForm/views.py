...
#                                   )
#         )
from OnlineForm.forms import SurveyForm, CategoryMetaInlineFormset, QuestionMetaInlineFormset
...
class SurveyCreateView(CreateView):
    ...
    def post(self, request, args=None, *kwargs):
        ...
        if form.is_valid() and question_meta_formset.is_valid():
            return self.form_valid(form, question_meta_formset)
        else:
            return self.form_invalid(form, question_meta_formset)

    def form_valid(self, form, question_meta_formset):
        print('*************************')
        self.object = form.save(commit=False)
        self.object.save()
        # saving ProductMeta Instances
    ...
    def form_invalid(self, form, question_meta_formset):
        print('???????????????????????')
        return self.render_to_response(
